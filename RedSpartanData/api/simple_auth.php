...
// Simple activity logger for this endpoint
function logActivitySimple($db, $userId, $action, $description = '') {
    try {
        $stmt = $db->prepare(
        ...
        error_log('logActivitySimple error: ' . $e->getMessage());
    }
}
...
function handleLogin($db) {
    ...
    try {
        ...
        if (!$user) {
            ...
            logActivitySimple($db, null, 'login_failed', 'Failed login: ' . $identifier);
        ...
        $isActive = true;
        if ($statusValue !== '') {
            if (in_array($statusValue, ['disable', 'disabled', 'inactive', 'blocked', 'suspended', '0'], true)) {
                $isActive = false;
            }
        }
        if (!$isActive) {
            sendError('This user account has been disabled by the admin or super admin. Please contact the administrator for assistance.', 403);
        }
        ...
        if (!$isValid) {
            ...
            // Log failed login with user id if available
            logActivitySimple($db, $user['id'] ?? null, 'login_failed', 'Invalid password');
        ...
        // Log successful login for any role
        logActivitySimple($db, $user['id'], 'login', 'User logged in');
...
function handleLogout($db) {
    ...
    if ($userId) {
        logActivitySimple($db, $userId, 'logout', 'User logged out');
    }
    // Clear session
    session_unset();
    session_destroy();
