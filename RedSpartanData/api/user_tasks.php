...
function getAssignedTasks() {
    ...
    try {
        ...
        if (in_array('assigned_by', $existingColumns)) {
            $selectFields[] = 'ta.assigned_by';
        }
        ...
        $sql = "SELECT " . implode(', ', $selectFields) . "
                ...
                AND (
                    ...
                    OR (LOWER(TRIM(ta.assigned_office)) = LOWER(?) AND u.campus = ? AND u.campus IS NOT NULL)
                )
        ...
        // Format tasks with deadline and priority information
        $now = new DateTime();
        $formattedTasks = [];
        foreach ($tasks as $task) {
            ...
            $formattedTask = [
                ...
                'assigned_office' => $task['assigned_office'],
                'description' => $task['description'] ?? 'No description provided',
                'assigned_date' => $task['assigned_date'],
                'status' => $task['status'],
                'task_status' => 'pending',
                'assigned_by_name' => $task['assigned_by_name'] ?? 'Admin',
                'assigned_by_campus' => $task['assigned_by_campus'] ?? null,
                ...
                'priority' => isset($task['priority']) && !empty($task['priority']) ? $task['priority'] : 'low',
