...
class UsersAPI {
    ...
    private function getAccessibleCampuses($adminCampus) {
        ...
        // Prefer dynamic hierarchy helper if available
        if (function_exists('getAccessibleCampusesForAdmin')) {
            return getAccessibleCampusesForAdmin($adminCampus);
        }

        // Fallback: original static mapping
    ...
    private function listUsers() {
        try {
            ...
            $perPageParam = isset($_GET['per_page']) ? $_GET['per_page'] : null;
            $perPage = null;
            if ($perPageParam !== null && strtolower((string)$perPageParam) !== 'all') {
            ...
            // Fetch page
            if ($perPage === null) {
                $listSql = 'SELECT id, username, name, role, status, campus, office, created_at, last_login ' . $base . ' ORDER BY created_at DESC';
            ...
            $metaPerPage = $perPage === null ? $total : $perPage;
            $metaTotalPages = $perPage === null ? 1 : (int)ceil($total / $perPage);
    ...
    private function createUser() {
        try {
            ...
            // Normalize status - accept 'disable' and map old statuses
            $statusLower = strtolower(trim($status));
            if (in_array($statusLower, ['disable', 'disabled', 'inactive', 'blocked', 'suspended'], true)) {
                $status = 'disable';
