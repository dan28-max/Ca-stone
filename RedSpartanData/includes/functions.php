...
/**
 * Get campus hierarchy from system settings (with sensible defaults).
 * Structure:
 * [
 *   'campuses' => [
...
function getCampusHierarchy() {
    ...
    if (!empty($settings['campus_hierarchy'])) {
        ...
        if (is_array($decoded) && isset($decoded['campuses']) && is_array($decoded['campuses'])) {
            $cachedHierarchy = $decoded;
            return $cachedHierarchy;
        }
    }

    // Default mapping (mirrors original hard-coded access rules)
    $cachedHierarchy = [
        'campuses' => [
            // Main parents
            ['name' => 'Pablo Borbon', 'parent' => null],
...
/**
 * Given an admin campus, return all campuses they should have access to
 * based on the campus hierarchy. Rules:
 * - Admin of campus X sees X + all campuses whose parent is X.
 */
function getAccessibleCampusesForAdmin($adminCampus) {
    $adminCampus = trim((string)$adminCampus);
    if ($adminCampus === '') {
        return [];
    }

    $hierarchy = getCampusHierarchy();
    $campuses = isset($hierarchy['campuses']) && is_array($hierarchy['campuses'])
        ? $hierarchy['campuses']
        : [];

    $accessible = [$adminCampus];

    foreach ($campuses as $item) {
        $name = isset($item['name']) ? trim((string)$item['name']) : '';
        $parent = isset($item['parent']) ? trim((string)$item['parent']) : '';

        if ($name !== '' && strcasecmp($parent, $adminCampus) === 0) {
            $accessible[] = $name;
        }
    }
